...
import { schema } from "./NewTruck";

const EditTruck = () => {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const { id: truck_id } = useParams();
  ...
  const truckInstance = useEntities({
    keys: ["trucks", truck_id],
  ...
  useEffect(() => {
    if (truckInstance.data) reset(truckInstance.data);
  }, [truckInstance.data]);
  ...
  return (
    <Box w={{ base: "100%", lg: "100%" }} m="auto" px="20px">
      <Heading mb={30}>Edit truck</Heading>
